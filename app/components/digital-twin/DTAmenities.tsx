// app/components/digital-twin/DTAmenities.tsx
"use client";
...
const POINTS = [
  "See on-site & nearby amenities on a unified view with walking/driving times from any unit.",
  "Filter by category and opening hours; spot live open/closed status and peak times (if available).",
  "Open amenity details to view photos, rules, capacity, and navigate with a shareable link.",
];
...
export default function DTAmenities({
  mediaSrc = "/suncity/amenities.jpg",
  videoSrc,
  title = "Explore Site Amenities",
  blurb = "Discover on-site and nearby amenities with real distances, opening hours, and categories — all inside the Digital Twin.",
  overlayTitle = "Amenity Map & Details",
  overlayText = "Browse gym, pool, playgrounds, parking, retail, and transit. See travel times from any unit, opening hours, photos, and access rules.",
}: Props) {
      ...
      <div className="relative z-[1] mx-auto max-w-[1450px] px-6 py-16 md:py-24">
        ...
        <motion.div variants={WRAP} initial="hidden" animate={controls} className="text-center mb-8 md:mb-12">
          ...
          <motion.h2 variants={ITEM} className="typo-h2-md mt-2">
            <span className="text-[#C6F24E]">Explore</span> Site Amenities
          </motion.h2>
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-12 md:gap-12">
          {/* LEFT — media window with a single info box overlay */}
              ...
              <div className="relative aspect-[16/9]">
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <span className="rounded-md bg-[#C6F24E] px-2 py-1 text-[11px] font-medium text-black/90">Distances</span>
                    <span className="rounded-md bg-white/10 px-2 py-1 text-[11px] text-white/80 ring-1 ring-white/15">Opening Hours</span>
                    <span className="rounded-md bg-white/10 px-2 py-1 text-[11px] text-white/80 ring-1 ring-white/15">Photos</span>
                  </div>
                </motion.div>
          ...
          {/* RIGHT — outlined number points (vertically centered text) */}
          <motion.ul variants={WRAP} initial="hidden" animate={controls} className="md:col-span-5 md:self-center space-y-8">
            {POINTS.map((t, i) => (
              <motion.li key={i} variants={ITEM}>
                <div className="flex items-center gap-5">
                  ...
                  {/* keep text vertically centered against the number tile */}
            ...
            <motion.div variants={ITEM} className="mt-4 flex flex-wrap gap-2">
              <span className="rounded-md bg-[#C6F24E] px-2.5 py-1 text-[11px] font-medium text-black/90">Gym</span>
              <span className="rounded-md bg-white/10 px-2.5 py-1 text-[11px] text-white/85 ring-1 ring-white/15">Pool</span>
              <span className="rounded-md bg-white/10 px-2.5 py-1 text-[11px] text-white/85 ring-1 ring-white/15">Playground</span>
              <span className="rounded-md bg-white/10 px-2.5 py-1 text-[11px] text-white/85 ring-1 ring-white/15">Parking</span>
              <span className="rounded-md bg-white/10 px-2.5 py-1 text-[11px] text-white/85 ring-1 ring-white/15">Retail</span>
              <span className="rounded-md bg-white/10 px-2.5 py-1 text-[11px] text-white/85 ring-1 ring-white/15">Transit</span>
            </motion.div>
          </motion.ul>
        </div>
      </div>
