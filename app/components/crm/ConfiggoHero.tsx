...
// >>> tweak this gap if you want more/less space above the hero:
const HEADER_OFFSET = "calc(var(--header-h, 72px) + 46px)";
const HERO_AREA_HEIGHT = "calc(100svh - (var(--header-h, 72px) + 12px))";
...
export default function ConfiggoHero() {
  ...
  return (
    <section
      ...
      style={{
        minHeight: "100svh",
        paddingTop: HEADER_OFFSET,        // space for header + small gap
        scrollMarginTop: HEADER_OFFSET,   // smooth anchor scroll below header
      }}
    >
      {/* background swirl */}
      ...
      <div
        className="relative z-[1] mx-auto flex max-w-[1450px] items-center px-6"
        style={{ minHeight: HERO_AREA_HEIGHT }}  // fill remaining viewport height
      >
        <div className="grid w-full grid-cols-1 items-center gap-12 md:grid-cols-12 md:gap-12">
          {/* LEFT — copy */}
          <motion.div variants={ITEM} initial="hidden" animate={controls} className="md:col-span-6">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="/contact" className="rounded-full bg-[#C6F24E] px-5 py-3 text-sm text-black transition hover:brightness-95">
                Get a demo
              </Link>
              <a href="#video" className="rounded-full border border-white/15 bg-white/10 px-5 py-3 text-sm text-white/90 backdrop-blur-sm transition hover:bg-white/15">
                See it in action
              </a>
            </div>

            <div className="mt-6 flex flex-wrap gap-2">
              {["Lead Routing", "Pipelines", "Inventory & Availability", "Omnichannel Inbox", "Reporting"].map((label) => (
                <span key={label} className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/70">
                  {label}
                </span>
              ))}
            </div>
          ...
          <div className="md:col-span-6">
            <Image
              ...
              priority
              sizes="100vw"
              className="h-auto w-full object-contain animate-in"
            />
          </div>
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        .animate-in { animation: fadeInUp 0.7s cubic-bezier(0.22, 0.61, 0.36, 1) both; }
      `}</style>
    </section>
  );
}
