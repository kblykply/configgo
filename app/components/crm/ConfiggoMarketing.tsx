...
export default function ConfiggoMarketing() {
  ...
  return (
    <section ref={ref} className="relative" id="marketing">
      {/* soft glow */}
          ...
          <motion.div variants={VISUAL} initial="hidden" animate={controls} className="md:col-span-7">
            <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-black/40 shadow-[0_40px_100px_rgba(0,0,0,0.6)]">
              ...
              <div className="relative aspect-[16/9] p-4 md:p-6">
                {/* JOURNEY BUILDER */}
                {tab === "journey" && <JourneyCanvas play={inView} />}

                {/* PERFORMANCE */}
                {tab === "performance" && <PerformancePanel play={inView} />}
...
/* -------------------- Journey canvas (nodes + animated connectors) -------------------- */
function JourneyCanvas({ play }: { play: boolean }) {
  return (
    <div className="h-full w-full rounded-xl border border-white/10 bg-white/[0.04] p-3 md:p-5 relative overflow-hidden">
      {/* nodes */}
      <Node x="6%"  y="10%"  Icon={Target} label="Trigger" sub="Source: Meta Ads" />
      <Node x="38%" y="12%"  Icon={Filter} label="Condition" sub="Budget ≥ 200k" />
      <Node x="72%" y="10%"  Icon={MessageCircle} label="WhatsApp" sub="Template: Intro + CTA" green />
      <Node x="38%" y="55%"  Icon={CalendarClock} label="Delay" sub="Wait 1 day" />
      <Node x="72%" y="55%"  Icon={Mail} label="Email" sub="Floor plan + booking" green />

      {/* connectors */}
      <Connector from={{x: "16%", y: "18%"}} to={{x: "46%", y: "18%"}} play={play} />
      <Connector from={{x: "54%", y: "18%"}} to={{x: "82%", y: "18%"}} play={play} />
      <Connector from={{x: "46%", y: "18%"}} to={{x: "46%", y: "63%"}} play={play} vertical />
      <Connector from={{x: "54%", y: "63%"}} to={{x: "82%", y: "63%"}} play={play} />
  ...
  return (
    <div
      className={[
        ...
        "w-[190px] bg-white/[0.06] ring-1 ring-white/10",
...
function Connector({
  ...
  // simple animated line using SVG stroke-dashoffset
  return (
    <svg
...
/* -------------------- Performance panel (bars + A/B card) -------------------- */
function PerformancePanel({ play }: { play: boolean }) {
  const stats = useMemo(
    () => [
