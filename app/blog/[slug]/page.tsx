...
// import type { PageProps } from "next"; // <-- SİL
...
export default async function BlogDetail(
  { params }: { params: Promise<{ slug: string }> } // <-- Promise tipi
) {
  const { slug } = await params; // <-- await et
  const post = getPostBySlug(slug);
  if (!post) notFound();

  return (
    <main className="min-h-svh bg-[var(--background)] text-white">
      <article className="mx-auto max-w-[1100px] md:max-w-[1200px] px-6 md:px-10 pt-24 md:pt-32 pb-16 md:pb-24">
        <Link href="/blog" className="text-sm text-white/70 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 rounded">
          ← Back to all
        </Link>

        <h1 className="mt-4 text-[40px] leading-[1.05] md:text-[56px]" style={{ fontFamily: "var(--font-heading)", fontWeight: 500 }}>
          {post.title}
        </h1>
        ...
        <div className="relative mt-8 h-[46svh] min-h-[320px] max-h-[640px] overflow-hidden rounded-3xl ring-1 ring-white/10 bg-white/[0.04]">
          <Image src={post.cover} alt={post.title} fill className="object-cover" priority />
          <div className="pointer-events-none absolute inset-0" style={{ boxShadow: `inset 0 -140px 200px -100px ${ACCENT}2a` }} />
        </div>
...
function Prose({ content }: { content: PostContent }) {
  return (
    ...
    >
      {content.map((b, i) => {
        if (b.type === "h2") return <h2 key={i} style={{ fontFamily: "var(--font-heading)" }}>{b.text}</h2>;
        if (b.type === "p") return <p key={i}>{b.text}</p>;
        if (b.type === "img") {
          return (
            <figure key={i} className="my-10 overflow-hidden rounded-xl border border-white/10 bg-white/5">
              <img src={b.src} alt={b.alt || ""} className="block w-full" />
              {"caption" in b && b.caption ? (
                <figcaption className="px-4 py-2 text-center text-sm text-white/60">{b.caption}</figcaption>
              ) : null}
            </figure>
          );
        }
        if (b.type === "quote") {
          return (
            <blockquote key={i} className="my-10 border-l-2 pl-4" style={{ borderColor: ACCENT }}>
              <p className="text-white/80">{b.text}</p>
              {"cite" in b && b.cite ? <cite className="mt-1 block text-sm text-white/60">— {b.cite}</cite> : null}
            </blockquote>
          );
        }
        if (b.type === "list") {
